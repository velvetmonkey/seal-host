...
def emit(phase: str, event: str, capacity: int, used: int, available: int, **extra: int) -> None:
    fields = {
        "phase": phase,
        "event": event,
    ...
    print("DISK_TELEMETRY " + " ".join(f"{key}={value}" for key, value in fields.items()), flush=True)
...
def main() -> int:
    ...
    try:
        ...
        print(f"DISK_TELEMETRY phase={args.phase} command_error={error}", file=sys.stderr)
        command_rc = 127
    else:
