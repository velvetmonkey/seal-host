...
Evaluates the REAL Lean decision core (`Ffi.modelInit` / `Ffi.modelStep`, i.e.
the private `initImpl` / `stepImpl` the proofs and the FFI exports both build on)
in the Lean INTERPRETER via a `#eval` under `lake env lean`. It is NOT the
compiled `.so` and NOT a reimplementation — it is the same Lean source the
theorems govern, run through Lean's own evaluator. The bridge diffs this against
the compiled native artifact to test that codegen preserves the proven decisions
+ audit bytes on the corpus.
...
`#eval` under `lake env lean` does not receive the process stdin):
  SEAL_CONF_ENV=<envelope-file> SEAL_CONF_PK=<pubkey> \
...
#eval show IO Unit from do
  let envPath := (← IO.getEnv "SEAL_CONF_ENV").getD ""
  let pk := (← IO.getEnv "SEAL_CONF_PK").getD ""
  let corpusPath := (← IO.getEnv "SEAL_CONF_CORPUS").getD ""
  let outPath := (← IO.getEnv "SEAL_CONF_OUT").getD ""
  let envelope ← IO.FS.readFile envPath
  let initOut ← modelInit envelope pk
