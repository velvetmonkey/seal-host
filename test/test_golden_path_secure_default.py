...
class GoldenPathSecureDefaultTests(unittest.TestCase):
    def test_signed_config_passes_and_tamper_is_denied_without_mode_flag(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="seal-secure-golden-path-") as directory:
            work = Path(directory)
            os.chmod(work, 0o700)
            ...
            try:
                filesystem.secure_default_preflight_leg(
                    trusted, config_pub, approval_pub, work
                )
            finally:
                filesystem.HOST = original_host

            observed = {row.name: (row.status, row.evidence) for row in filesystem.CHECKS}
            self.assertEqual(observed["secure-default signed config"][0], "PASS")
            self.assertEqual(observed["secure-default tamper denial"][0], "PASS")
            self.assertIn("trusted config rejected", observed["secure-default tamper denial"][1])
