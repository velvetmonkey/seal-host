...
|-------|---------|----------------------|--------|
| The deployable MCP host that puts the proven Seal rulebook between an agent and real tools. Guarded calls stop until a matching live approval record exists for that exact request (record origin/custody assumed — next row). | Yes (implemented + tested) | rust/src/main.rs (Block path + short-circuit), providers.rs (Ed25519TokenProvider + ControlFile), demo/see_the_loop.py + synthetic_ledger (shows block + flow/refused) | keep |
| Policy-covered request-effects reach the child only after every applicable Lean kernel returns Allow; effects configured as guarded additionally require a matching live approval record; seam failures block; every decision emits replayable evidence. (The theorem binds an approval record in state, not a human: signer identity/custody is a declared assumption — see the truth box.) | Yes (proven at kernel + tested at host) | seal-host/CLAIMS.md + rust integration tests + Lean Host/Step.lean + conformance_bridge.mjs; guarded_allow_iff_live (SealCore/Safety.lean) | keep |
