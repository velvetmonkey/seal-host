...
| One-command demo over synthetic shows block → signed record → flow or refused → receipt. | Yes (shipped demo described in doc; produces visible BLOCK hex + SYNTHETIC or 'refused' when run) | demo/see_the_loop.py (prints BLOCK, SYNTHETIC side-effect or host-emitted refused, === PASS ===); run in glowup evidence | keep |
