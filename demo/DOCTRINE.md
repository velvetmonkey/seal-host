...
kernel, pinned theorem references, authorization-decision path, and a green in-run
verification result on its applicable lane. `BLOCK` is rendered as the
human-facing `DENY` while the trace retains the authorization-decision vocabulary in
`receipt_verdict`.
...
`seal verify` replays every authorization decision assigned to this lane; any
...
The finalizer verifies every Lane A authorization decision, flips one byte in an emitted
standalone authorization decision and requires verification to fail, restores the original
...
The fixed claim scope is deliberately narrow: the authorization decision attests the
...
are assigned to whole authorization decisions: because the authorization-decision/verifier pair does not carry
and replay votes or Linear grant events, all three combined C3 authorization decisions are
...
action can ever occur. The trigger authorization decision is reproduced in Lane A;
...
The first authorization decision is therefore Lane A while the second is honestly Lane B and
...
byte-mismatch, byte-flip, and byte-exact restoration controls. The authorization decision
...
and Budget spend uncommitted and never reaches the adapter. All seven authorization decisions
are Lane B because their composite bytes depend on omitted Consensus votes and
Linear grant events, with the later authorization decisions also carrying session state. The
