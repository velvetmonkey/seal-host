...
//!    manifest-pinned Phase-M `mcp-seal` package (`316d741`, carrying
//!    `SealV2.EffectEnvelope`) and diffs
...
//!
//! Expectation provenance: refreshed for the Phase-M cutover against the live
//! manifest pin `316d741` (toolchain v4.28.0). Present claims now carry the
//! `optMeta` byte; the shared corpus itself is unchanged. The `golden-fable`
//! line is additionally pinned below to the exact hex that mcp-seal-dev's
//! `SealV2/EffectEnvelope.lean` freezes under `#guard_msgs`, so the frozen file
//! cannot silently drift from the spec repo's own pin.
...
/// `#guard_msgs` at `316d741` (and `rust/tests/envelope_v23.rs` pins as the
/// golden vector). Anchor: the expectation FILE must agree with the spec
/// repo's own frozen literal on this vector.
const LEAN_GUARD_MSGS_GOLDEN_HEX: &str = "";
...
/// (`316d741`, containing Phase M). Override with
