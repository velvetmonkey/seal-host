...
//! manifest pin `316d741` (toolchain v4.28.0). Present claims carry the
//! `optMeta` byte, and metadata-present additions are generated by the same
//! Lean spec lane. The `golden-fable` line is additionally pinned below to the
//! exact hex that mcp-seal-dev's `SealV2/EffectEnvelope.lean` freezes under
//! `#guard_msgs`, so the frozen file cannot silently drift from the spec
//! repo's own pin.
...
fn optional_value(v: &Value, key: &str) -> Option<Value> {
    v.as_object()
        .unwrap_or_else(|| panic!("corpus value containing {key} must be an object"))
        .get(key)
        .cloned()
}
        ...
        .map(|v| {
            ...
            let effect = if e["effect"].is_null() {
                ...
                Some(EffectClaim {
                    ...
                    metadata: optional_value(&e["effect"], "metadata"),
                    request_state: optional_value(&e["effect"], "requestState"),
                    input_responses: optional_value(&e["effect"], "inputResponses"),
                })
            };
    ...
    corpus_vectors()
        ...
        .map(|(name, line, authority, envelope)| {
            (
                name.clone(),
                hex::encode(
                    effect_message(authority, envelope, line).unwrap_or_else(|error| {
                        panic!("vector {name}: Rust encoder rejected corpus input: {error}")
                    }),
                ),
            )
        })
