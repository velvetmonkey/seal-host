...
| **Conditional single-link mediation:** for a strict MCP server spawned behind this host, every strict `tools/call` received on this host's stdio link is classified and decided before that host writes the call to that child. A6 closed for the signed-token production channel by the durable Rust replay store | THREAT_MODEL.md capstone | mixed (see below) | A1-A4 plus Rust replay-store TCB; **the absence of bypass paths is an ASSUMPTION, not an enforced property** | non-mediated routes admitted in `THREAT_MODEL.md` (direct shell/network, alternate MCP config, cached handles, in-process calls, subprocesses, non-`tools/call` methods); strict-monitor/lenient-child parser differential passes malformed or mis-spelled calls through as non-call traffic (`RUST_BRIDGE.md:18-24`, `Host/Canonical.lean:55-60`); response egress unmediated; legacy control-file channel cross-restart replay remains demo-only residual | yes (verbatim only) |
