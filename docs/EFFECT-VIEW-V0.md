...
`effect_view` is a non-authoritative, receipt-only projection emitted by the
...
advisory only. `policy_hash` reuses the receipt's existing canonical hash of
the trusted config; neither field is signed by the principal envelope.

If the receipt-side parser cannot recover the MCP call, `effect_view` is
...
store is deliberately outside this receipt-only change.

## Freshness and record commitments

The runtime session and idempotency key are passive receipt fields. They do
...
tamper-evident Lean chain payload. It changes only the sidecar decision
receipt assembled in `rust/src/decision_receipt.rs`; the Lean-emitted record
