...
| Receipt assembly and persistence | Rust decision-receipt producer | Trusted additive producer. Receipt verification reproduces decision bytes; `host_identity` identifies native artifacts but does not close Lane C equivalence. Sink failure is fail-closed. |
