...
| T6 | **Wall clock + A3 state + replay store** (`a3.rs`, `replay_store.rs`) — nonce set, Ed25519 signed-token SQLite durability, TTL, future-skew, and database-wide lineage validation against authority-signed config. | Startup refuses missing, unsupported, or mismatched schema/namespace lineage. Nonce insert is write-ahead before Lean sees the approval; SQLite runs WAL + `synchronous=FULL`. Legacy control-file replay remains in-memory/demo-only. |
...
| Which binary ran (identity, not equivalence) | wasm sha256 pin | `seal-host/wasm-spike/verified/PROVENANCE.txt` (`seal.wasm` pin `c9f32b00543c…8eda`) | `sha256sum wasm-spike/verified/seal.wasm` |
