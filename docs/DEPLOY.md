...
- Bash on Linux and a checkout of this public repository.
- An already working host binary from this exact checkout. This page does not
  provide a supported build or download route.
...
There is no archive to download or verify. The release workflow and verifier
code define a proposed six-subject provenance contract, but no run has
published those subjects. For the recorded local path only, identify an
already working binary from this checkout as `SEAL_BIN`.
