...
|---|---|---|---|---|
| `safety` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.safety_always_registered` | ✅ always (64/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Monkey, 2026-07-16) — yes — scaffolded by `seal init`; present in every recipe | ✅ derived — shown denying at all 64 topologies (`rust/tests/topology_matrix.rs`) |
| `temporal` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.temporal_always_registered` | ✅ always (64/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Monkey, 2026-07-16) — yes — `seal add-kernel T`; emitted by the prod-db recipe | ✅ derived — shown denying at all 64 topologies (`rust/tests/topology_matrix.rs`) |
| `consensus` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.consensus_registered_iff` | ⚙️ config-gated (32/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Monkey, 2026-07-16) — yes — `seal add-kernel C`; emitted by the deploy recipe | ✅ derived — shown denying at all 32 active topologies and not gating at the 32 inactive ones (`rust/tests/topology_matrix.rs`) |
| `convergence` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.convergence_registered_iff` | ⚙️ config-gated (32/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Monkey, 2026-07-16) — yes — `seal add-kernel V`; emitted by the mesh recipe | ✅ derived — shown denying at all 32 active topologies and not gating at the 32 inactive ones (`rust/tests/topology_matrix.rs`) |
| `calibration` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.calibration_registered_iff` | ⚙️ config-gated (32/64) derived — evaluated over all 64 configs; double-gated (present AND enabled) | ⚠️ ASSERTED (Monkey, 2026-07-16) — experimental only — `seal add-kernel K --experimental`; no recipe emits it | ✅ derived — shown denying at all 32 active topologies and not gating at the 32 inactive ones (`rust/tests/topology_matrix.rs`); disabled-vs-absent exercised at all 16 inactive topologies |
| `linear` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.linear_registered_iff` | ⚙️ config-gated (32/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Monkey, 2026-07-16) — yes — `seal add-kernel L`; emitted by the deploy recipe | ✅ derived — shown denying at all 32 active topologies and not gating at the 32 inactive ones (`rust/tests/topology_matrix.rs`) |
| `budget` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.budget_registered_iff` | ⚙️ config-gated (32/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Monkey, 2026-07-16) — yes — `seal add-kernel B`; emitted by the prod-db and token-governor recipes | ✅ derived — shown denying at all 32 active topologies and not gating at the 32 inactive ones (`rust/tests/topology_matrix.rs`) |
| `principal_budget` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.principal_budget_registered_iff`, `Ffi.registryFor_kernels_principal_irrelevant` | ⚙️ config-gated (32/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Fable (builder — PENDING operator countersign at frisk), 2026-07-19) — no — hand-authored `principals` JSON only; no `seal add-kernel` support (V2.1, DX pending) | ✅ derived — shown denying at all 32 active topologies and not gating at the 32 inactive ones (`rust/tests/topology_matrix.rs`) |
| `consensus-bytes` | ✅ derived — `Ffi.registryFor_kernels`, `Ffi.byteConsensus_never_registered` | ❌ never (0/64) derived — evaluated over all 64 configs | ⚠️ ASSERTED (Monkey, 2026-07-16) — no — no config section exists | ❌ untested in deployment — no deployable topology exists (derived from wired: never) |
...
- `principal_budget` — **no — hand-authored `principals` JSON only; no `seal add-kernel` support (V2.1, DX pending)**
  ⚠️ ASSERTED by Fable (builder — PENDING operator countersign at frisk), 2026-07-19. The shipped policy DX (seal-assurance-kit, frozen pin) predates V2.1: `seal add-kernel` has no PB letter and no recipe emits a `principals` section, so the only authoring path is hand-writing the section per the schema/CONFIG (id + 64-hex pubkey registry, per-principal budgets — `seal validate` lints it). Asserted from the kit pin's vocabulary, not by running new DX; re-assert when the kit gains PB authoring.
